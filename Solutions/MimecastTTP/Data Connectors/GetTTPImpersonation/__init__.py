...
def main(mytimer: func.TimerRequest, checkpoint2: str) -> str:
    ...
    start_date = checkpoint2 if checkpoint2 else DateHelper.get_utc_time_in_past(days=7)
    ...
    ttp_impersonation_parser = TTPImpersonationParser()
    parsed_logs = []

    try:
        while has_more_logs:
            model = GetTTPImpersonationLogsRequest(mimecast_start_date, mimecast_end_date, next_token)
            response = request_helper.send_post_request(model.payload, MimecastEndpoints.get_ttp_impersonation_logs)
            response_helper.check_response_codes(response, MimecastEndpoints.get_ttp_impersonation_logs)
            ...
            parsed_logs.extend(ttp_impersonation_parser.parse(logs=success_response[0]["impersonationLogs"]))
    except MimecastRequestError as e:
        logging.error(
            "Failed to get TTP Impersonation logs from Mimecast.", extra={"request_id": request_helper.request_id}
        )
    ...
    try:
        if parsed_logs:
            ...
            log_type = "MimecastTTPImpersonation"
            ...
            logging.info("There are no TTP Impersonation logs for this period.")
            return mimecast_end_date
    except AzureMonitorCollectorRequestError as e:
        logging.error(
            "Failed to send TTP Impersonation to Azure Sentinel.", extra={"request_id": request_helper.request_id}
        )
        e.request_id = request_helper.request_id
        raise e
    except Exception as e:
