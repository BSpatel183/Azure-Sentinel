...
class RequestHelper:
    ...
    def make_request_headers(self, request_uri):
        ...
        hmac_sha1 = hmac.new(
            base64.b64decode(self.secret_key), unsigned_auth_header.encode(), digestmod=hashlib.md5
        ).digest()
    ...
    def make_refresh_token_request_headers(self):
        ...
        headers = {
            "Authorization": "Basic-Cloud {encoded_header}".format(
                encoded_header=authorization_header_value.decode("ascii")
            ),
            "x-mc-app-id": self.app_id,
            "Content-Type": "application/json",
